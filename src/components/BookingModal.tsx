...
export function BookingModal({ isOpen, onClose, onSuccess, userId, empresaId }: BookingModalProps) {
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      const { data: activeBarbers } = await supabase
        .from('barbers_salao')
        .select('id, barber_services_salao(service_id)')
        .eq('active', true)
        
      const allowedServiceIds = new Set<string>()
      activeBarbers?.forEach((b: any) => {
        b.barber_services_salao?.forEach((bs: any) => allowedServiceIds.add(bs.service_id))
      })
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          name, 
          photo_url, 
          barber_services_salao!inner(service_id)
        `)
        .eq('active', true)
        .eq('barber_services_salao.service_id', serviceId)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white border border-green-100 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-green-500 hover:text-green-800"
        >
          <X className="w-5 h-5" />
        </button>

        <div className="p-6 border-b border-green-100">
          <h2 className="text-xl font-bold text-green-950">Novo Agendamento</h2>
          <p className="text-green-600 text-sm">Passo {step === 3 && barbers.length <= 1 ? 2 : step} de {barbers.length > 1 ? 3 : 2}</p>
        </div>
        ...
        <div className="p-6">
          ...
          {step === 1 && (
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              <Label className="text-green-800">Selecione o Serviço</Label>
              ...
              >
                {barbersLoading ? 'Carregando...' : 'Continuar'}
              </Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              <div className="flex items-center justify-between mb-2">
                 <Label className="text-green-800">Selecione o Profissional</Label>
                 <button onClick={() => setStep(1)} className="text-green-700 text-xs hover:underline">Voltar</button>
              </div>
                 ...
                 <div 
                   ...
                   className={`p-4 rounded-xl border cursor-pointer flex items-center gap-4 transition-all ${
                     selectedBarber?.id === b.id 
                       ? 'border-green-700 bg-green-50' 
                       : 'border-green-100 bg-white hover:bg-green-50 hover:border-green-300'
                   }`}
                 >
                   {b.photo_url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={b.photo_url} alt={b.name} className="w-12 h-12 rounded-full object-cover border border-green-200" />
                   ) : (
                      <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center font-bold text-green-500 border border-green-200">
                        <User className="w-6 h-6" />
                      </div>
                   )}
                   <span className="font-medium text-green-950 text-lg">{b.name}</span>
                 </div>
              ))}
            ...
            <div className="space-y-6">
              <div className="p-4 rounded-xl bg-green-50 border border-green-100">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-sm text-green-600">Resumo:</span>
                  <button onClick={() => setStep(barbers.length > 1 ? 2 : 1)} className="text-green-700 text-xs hover:underline flex flex-col items-end">
                    Voltar
                  </button>
                </div>
                <p className="font-medium text-green-950">{selectedService?.name}</p>
                {selectedBarber && (
                  <p className="font-medium text-green-700 text-sm mt-1 flex items-center gap-1">
                    <User className="w-3 h-3 text-green-700" /> Com: {selectedBarber.name}
                  </p>
                )}
                <div className="flex justify-between items-end mt-2">
                  ...
                  <p className="text-green-800 font-bold">R$ {selectedService?.price}</p>
                </div>
              </div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (config?.closed_dates?.includes(val)) {
                      alert('O salão estará fechado nesta data.')
                      setDate('')
                      return
                    }
                    if (val && config?.open_days) {
                       ...
                       if (!config.open_days.includes(dayOfWeek)) {
                          alert('O salão não abre neste dia da semana.')
                          setDate('')
                          return
                       }
                    }
                  ...
                  className="bg-white border-green-200 text-green-900 focus:border-green-700"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                {!date ? (
                  <p className="text-sm text-green-500 italic">Selecione uma data primeiro</p>
                ) : (
                          ...
                          <button
                            ...
                            className={`py-2 text-sm rounded-lg border transition-all ${
                              isOccupied
                                ...
                                : 'bg-white border-green-100 text-green-700 hover:border-green-400 hover:text-green-900'
                            }`}
                          >
                            {slot}
                          </button>
